...
export default function NewRecipe() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      const path = `recipe-images/${uuid()}.${ext}`;
      ...
      setTitle(""); setMinutes(''); setTags(""); setIngredients(""); setSteps(""); setFile(null);
    } catch (err: any) {
  ...
  return (
    <main style={{maxWidth:760, margin:"40px auto", fontFamily:"system-ui"}}>
      ...
      <form onSubmit={handleSave} style={{display:"grid", gap:12}}>
        ...
        <label>Total minutes <input type="number" value={minutes} onChange={e=>setMinutes(e.target.value === '' ? '' : Number(e.target.value))} /></label>
        <label>Tags (comma separated) <input value={tags} onChange={e=>setTags(e.target.value)} placeholder="vegan,quick" /></label>
        <label>Ingredients <textarea rows={5} value={ingredients} onChange={e=>setIngredients(e.target.value)} placeholder={"1 cup flour\n2 eggs\n..."} /></label>
        <label>Steps <textarea rows={6} value={steps} onChange={e=>setSteps(e.target.value)} placeholder={"Preheat oven...\nMix...\nBake..."} /></label>
        <label>Image <input type="file" accept="image/*" onChange={e=>setFile(e.target.files?.[0] || null)} /></label>
        <button disabled={saving}>{saving ? "Saving..." : "Save"}</button>
      </form>
      {message && <p style={{marginTop:12}}>{message}</p>}
      <p style={{marginTop:16}}><a href="/recipes">Go to /recipes →</a></p>
    </main>
  );
}
